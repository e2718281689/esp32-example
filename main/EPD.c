...
void Wirte_EPD(uint8_t *picData, uint8_t x, uint8_t y, uint8_t nec)
{
    if (nec)
    {
        picData[(211 - x) * 13 + (uint8_t)(y / 8)] &= ~(1 << y % 8); //将X的第Y位清0
    }
    else
    {
        picData[(211 - x) * 13 + (uint8_t)(y / 8)] |= 0x01 << (y % 8); //将X的第Y位置1
    }
}
void Wirte_EPD_pieco(uint8_t *picData, uint8_t x, uint8_t y, uint8_t nec)//块写入
{
    picData[(211 - x) * 13 + (uint8_t)(y / 8)] = nec;
}
void lcd_chkstatus(void)//等待
{
    unsigned char busy;
    do
    {
...
void PIC_display_Clean(void)//清屏
{
    unsigned int i;
...
void PIC_display(uint8_t *picData_old,uint8_t *picData_new)//白红色画面
{
    EPD_W21_WriteCMD(0x10); // Transfer old data
...
void PIC_display_White(uint8_t *picData)//白色画面
{
    EPD_W21_WriteCMD(0x10); // Transfer old data
...
void PIC_display_Red(uint8_t *picData)//红色画面
{    
    EPD_W21_WriteCMD(0x10); // Transfer old data
    for (int i = 0; i < 2888; i++)
    {
